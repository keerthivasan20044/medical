...
export default function AdminMedicines() {
  return (
    <PageShell 
      title="Medicine List" 
      subtitle="Manage medicines and stock levels across Karaikal."
      icon={Pill}
      actions={
        <button className="h-16 px-10 bg-brand-teal text-[#0a1628] font-syne font-black text-xs rounded-2xl flex items-center gap-3 active:scale-95 shadow-4xl italic uppercase tracking-wider group">
          <Plus size={18} className="group-hover:rotate-90 transition-transform" /> Add New Medicine
        </button>
      }
    >
      <div className="p-20 space-y-16">
         <div className="flex flex-col md:flex-row gap-8 items-center justify-between border-b border-black/[0.03] pb-12">
            <h3 className="font-syne font-black text-3xl text-[#0a1628] uppercase italic leading-none flex items-center gap-6 shrink-0">
               <div className="h-2 w-16 bg-brand-teal rounded-full" /> Medicine Inventory
            </h3>
            <div className="flex gap-4 w-full md:w-auto">
               <div className="h-16 flex-1 md:w-96 min-w-[300px] bg-gray-50/50 border border-black/[0.03] rounded-2xl flex items-center px-8 text-[#0a1628] focus-within:border-brand-teal transition-all group">
                  ...
                  <input type="text" placeholder="Search medicines..." className="bg-transparent flex-1 px-4 font-dm italic font-bold text-xl outline-none" />
               </div>
         ...
         <div className="overflow-hidden border border-black/[0.03] rounded-[3rem]">
            <table className="w-full text-left">
               <thead className="bg-[#0a1628] text-white">
                  <tr>
                     {['Medicine Name', 'Total Stock', 'Price', 'Pharmacies', 'Status', 'Actions'].map(h => (
                  ...
                  {[
                     ...
                     <tr key={i} className="group hover:bg-gray-50/50 transition-colors">
                        <td className="p-10">
                           ...
                           <div className="text-[10px] text-gray-300 font-black uppercase tracking-widest italic leading-none">MED-KKL-UPDATE-{1024+i}</div>
                        </td>
         ...
         <div className="grid lg:grid-cols-3 gap-12 pt-12">
            {[
               { label: 'Total Medicines', val: '5,240 Items', icon: Database },
               { label: 'Verified Manufacturers', val: '142 Sources', icon: ShieldCheck },
               { label: 'Stock Health', val: '98.2% Active', icon: Activity }
            ].map(stat => (
