...
export default function PharmacySettings() {
  return (
    <PageShell 
      ...
      subtitle="Modify your pharmacy information and operational settings."
      icon={Settings}
      actions={
        <button className="h-16 px-10 bg-brand-teal text-[#0a1628] font-syne font-black text-xs rounded-2xl flex items-center gap-3 active:scale-95 shadow-4xl italic uppercase tracking-wider group">
          <Save size={18} className="group-hover:animate-pulse" /> Save Settings
        </button>
      }
    >
      <div className="p-20 grid lg:grid-cols-2 gap-24">
         <div className="space-y-12">
            ...
            <div className="space-y-8">
               {[
                  { label: 'Pharmacy Name', val: 'Apollo Pharmacy', icon: Store },
                  { label: 'Location', val: 'Karaikal New Colony', icon: Globe },
                  { label: 'Contact Number', val: '+91 94432 XXXXX', icon: Phone },
                  { label: 'Working Hours', val: '08:00 AM - 10:00 PM', icon: Clock }
               ].map(item => (
         ...
         <div className="space-y-12">
            <h3 className="font-syne font-black text-4xl text-[#0a1628] uppercase italic tracking-tighter leading-none flex items-center gap-6">
               <div className="h-2 w-16 bg-brand-teal rounded-full" /> Security & Privacy
            </h3>
            ...
            <div className="grid grid-cols-1 gap-8">
               ...
               <div className="bg-brand-teal/5 border-2 border-dashed border-brand-teal/20 p-12 rounded-[4rem] space-y-6">
                  <div className="flex items-center gap-4 text-brand-teal font-syne font-black text-sm uppercase italic">
                     <Heart size={24} /> HEALTH & WELLNESS COMPLIANCE
                  </div>
                  <p className="text-[#0a1628]/60 font-dm italic font-bold">Changes to core pharmacy information may require verification by administrative staff within 24 hours.</p>
               </div>
            </div>
         </div>
      </div>
