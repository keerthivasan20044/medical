...
import { IndianRupee, TrendingUp, Calendar, ArrowDownCircle, Download, Wallet, ArrowUpRight } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

const EARNINGS_DATA = [
  { date: '2024-04-14', amount: 4200 },
  { date: '2024-04-15', amount: 3800 },
  { date: '2024-04-16', amount: 5100 },
  { date: '2024-04-17', amount: 4800 },
  { date: '2024-04-18', amount: 6200 },
  { date: '2024-04-19', amount: 5900 },
  { date: '2024-04-20', amount: 7500 },
];

const TRANSACTIONS = [
  { id: 'TXN-8821', order: 'ORD-5542', type: 'Credit', amount: 1240, date: 'Today 14:30', status: 'Pending' },
  { id: 'TXN-8820', order: 'ORD-5541', type: 'Credit', amount: 850, date: 'Today 12:15', status: 'Completed' },
  { id: 'TXN-8819', order: 'ORD-5540', type: 'Credit', amount: 2100, date: 'Today 10:45', status: 'Completed' },
  { id: 'STL-102', order: '-', type: 'Settlement', amount: -42500, date: 'Yesterday', status: 'Settled' },
];
    ...
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-xs font-dm font-bold text-navy/40 uppercase tracking-widest mt-1 italic">Node Financial Synchronization</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm relative overflow-hidden group">
           ...
           <h4 className="text-4xl font-syne font-black italic tracking-tighter text-navy relative z-10">₹42,850.40</h4>
           <div className="mt-6 flex items-center gap-2 text-emerald-600 font-dm font-bold text-xs relative z-10">
              <ArrowUpRight size={14} /> +₹4,200 today
           </div>
        </div>
        ...
        <div className="bg-navy p-10 rounded-[3.5rem] text-white relative overflow-hidden group">
           ...
           <p className="text-[10px] font-black text-brand-teal uppercase tracking-widest italic mb-2 relative z-10">Total Revenue (MTD)</p>
           <h4 className="text-4xl font-syne font-black italic tracking-tighter text-white relative z-10">₹1,24,500</h4>
        ...
        <div className="bg-brand-teal p-10 rounded-[3.5rem] text-navy relative overflow-hidden group">
           ...
           <h4 className="text-4xl font-syne font-black italic tracking-tighter text-navy relative z-10">₹12,450</h4>
           <div className="mt-6 flex items-center gap-2 text-navy/40 font-dm font-bold text-xs relative z-10">
              <ArrowDownCircle size={14} /> 10% Protocol Flat
           </div>
        </div>
      </div>
      ...
      <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex items-center gap-2 text-[10px] font-black text-navy/40 uppercase tracking-widest italic">
             <Calendar size={14} /> Last 7 Cycles
          </div>
        </div>
          ...
          { 
            ...
            render: (val) => (
              <span className={`text-[10px] font-black uppercase tracking-widest italic ${
                val === 'Credit' ? 'text-emerald-500' : 'text-blue-500'
              }`}>{val}</span>
            )
          },
          { 
          ...
          { 
            ...
            render: (val) => (
              <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest w-fit border ${
                val === 'Completed' || val === 'Settled' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'
              }`}>
                {val}
              </div>
            )
