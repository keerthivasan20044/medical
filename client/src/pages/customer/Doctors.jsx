...
export default function Doctors() {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      console.warn('Doctor update failed, using mock registry...', err);
  ...
  const getDoctorSpec = (doc) => doc.spec || doc.specialization || doc.doctorProfile?.specialization || 'General Physician';
  const getDoctorImage = (doc) => doc.image || doc.avatar?.url || `https://ui-avatars.com/api/?name=${encodeURIComponent(doc.name || 'Doctor')}&background=028090&color=fff`;
  const getDoctorRating = (doc) => doc.rating || doc.doctorProfile?.rating || 4.8;

  return (
    <div className="bg-[#f8fafc] min-h-screen pb-28 md:pb-40 overflow-x-hidden">
      {/* Medical Hub Hero */}
      <section className="bg-[#0a1628] text-white pt-16 md:pt-24 pb-28 md:pb-48 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-10 relative z-10 text-center">
          <div className="flex flex-col items-center space-y-5 md:space-y-8">
            <div className="px-4 md:px-5 py-2 bg-brand-teal/10 border border-brand-teal/20 rounded-xl md:rounded-2xl w-fit flex items-center gap-2 text-[9px] md:text-[10px] font-black text-brand-teal uppercase tracking-[0.14em] md:tracking-[0.3em] italic">
              <Verified className="animate-pulse" size={14} /> CERTIFIED PRACTITIONERS
            </div>
            <h1 className="font-syne font-black text-4xl sm:text-5xl md:text-7xl lg:text-9xl text-white leading-tight md:leading-[0.9] tracking-normal md:tracking-tighter uppercase italic break-words max-w-full">
               {t('consultDoctorsTitle')}
            </h1>
            <p className="text-white/40 font-dm text-base md:text-2xl italic max-w-3xl leading-relaxed px-0 md:px-6">
               {t('consultDoctorsSubtitle')}
            </p>
          </div>
          
          <div className="max-w-4xl mx-auto relative group mt-8 md:mt-16 px-0 md:px-6">
             <div className="absolute left-4 md:left-10 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#028090] transition">
             ...
             <input 
                ...
                className="w-full h-14 md:h-24 pl-12 md:pl-24 pr-4 md:pr-64 rounded-2xl md:rounded-[3rem] border border-white/10 bg-white/5 backdrop-blur-3xl font-dm text-base md:text-2xl italic text-white shadow-4xl focus:outline-none focus:ring-4 focus:ring-[#028090]/20 transition-all placeholder:text-[12px] md:placeholder:text-2xl placeholder:italic placeholder-white/20"
             />
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 -mt-16 md:-mt-24 relative z-20">
         <div className="bg-white border border-gray-100 rounded-2xl md:rounded-[3.5rem] p-3 md:p-10 shadow-4xl flex items-center gap-3 md:gap-4 overflow-x-auto no-scrollbar">
            <div className="h-11 w-11 md:h-16 md:w-16 bg-gray-50 rounded-xl md:rounded-2xl flex items-center justify-center text-gray-400 shrink-0 shadow-inner"><Filter size={18} /></div>
            {SPECIALTIES.map(s => (
               <button 
                 key={s} 
                 onClick={() => setActiveSpec(s)}
                 className={`px-4 md:px-10 h-11 md:h-16 rounded-xl md:rounded-[2rem] font-syne font-black text-[9px] md:text-[10px] uppercase tracking-wider md:tracking-widest transition-all duration-500 whitespace-nowrap shadow-soft active:scale-95 ${activeSpec === s ? 'bg-[#0a1628] text-white shadow-4xl md:scale-110 md:translate-x-1' : 'bg-gray-50 text-gray-400 hover:bg-white border border-gray-100'}`}
               >
                  {s}
               </button>
            ))}
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 py-12 md:py-24">
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 md:gap-8 mb-8 md:mb-12 px-0 md:px-2">
            <div className="space-y-2 min-w-0">
               <h2 className="font-syne font-black text-[#0a1628] text-2xl md:text-5xl uppercase tracking-normal md:tracking-tighter italic break-words">{filteredDocs.length} Specialists</h2>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5 md:gap-12">
            <AnimatePresence mode="popLayout">
               {filteredDocs.map((doc, idx) => (
                  <motion.div 
                    ...
                    className="group bg-white rounded-2xl md:rounded-[4rem] border border-gray-100 p-5 md:p-12 shadow-soft hover:shadow-4xl hover:border-brand-teal/20 transition-all duration-700 relative flex flex-col h-full overflow-hidden"
                  >
                     <div className="absolute top-0 right-0 h-40 w-40 bg-brand-teal opacity-[0.02] rounded-full blur-[60px]" />
                     <div className="absolute top-5 right-5 md:top-8 md:right-8 flex flex-col gap-3 relative z-10">
                        <div className="h-11 w-11 md:h-14 md:w-14 rounded-xl md:rounded-2xl border border-gray-100 bg-white/50 backdrop-blur-md flex items-center justify-center text-gray-300 hover:text-red-500 hover:shadow-2xl transition-all duration-500 cursor-pointer shadow-sm active:scale-95"><Heart size={18} /></div>
                     </div>
                     
                     <div className="flex flex-col items-center text-center space-y-5 md:space-y-8 relative z-10">
                        <div className="relative">
                           ...
                           <img src={getDoctorImage(doc)} alt={doc.name} className="h-28 w-28 md:h-40 md:w-40 rounded-[2rem] md:rounded-[3.5rem] object-cover ring-8 md:ring-[12px] ring-gray-50 p-1.5 md:p-2 group-hover:scale-105 transition duration-1000 relative grayscale-[0.2] group-hover:grayscale-0" />
                        ...
                        <div className="space-y-3 min-w-0">
                           <h3 className="font-syne font-black text-[#0a1628] text-xl md:text-3xl group-hover:text-brand-teal transition-all duration-500 leading-tight md:leading-none uppercase tracking-normal md:tracking-tighter italic break-words">{doc.name}</h3>
                        ...
                        <div className="flex items-center gap-3 text-[11px] font-black text-amber-500 italic">
                           <Star size={18} fill="currentColor" /> {getDoctorRating(doc)} <span className="text-gray-200 font-black uppercase tracking-widest border-l border-gray-100 pl-3 ml-1">({doc.consultations || 0} consultations)</span>
                        </div>
                         ...
                         <div className="w-full space-y-6">
                            ...
                            <div className="flex items-center justify-between px-2">
                               ...
                               <div className="text-[10px] font-black text-[#0a1628] uppercase italic truncate max-w-[120px]">{(doc.hospital || doc.clinic || 'Karaikal Clinic').split(',')[0]}</div>
                            </div>
                         </div>

                         <div className="w-full flex flex-col gap-3 pt-4 mt-auto">
                            <Link to={`/doctors/${getDoctorId(doc)}`} className="w-full h-14 md:h-16 bg-[#0a1628] text-white rounded-xl md:rounded-[1.5rem] font-syne font-black text-[10px] uppercase tracking-[0.14em] md:tracking-[0.2em] hover:bg-brand-teal shadow-soft hover:shadow-4xl transition-all duration-500 flex items-center justify-center gap-3 group/btn relative overflow-hidden">
                            ...
                            <button className="w-full h-12 md:h-14 border-2 border-brand-teal/10 bg-white text-brand-teal rounded-xl md:rounded-[1.5rem] font-syne font-black text-[10px] uppercase tracking-[0.14em] md:tracking-[0.2em] hover:bg-brand-teal/5 transition-all duration-500 flex items-center justify-center gap-3">
         ...
         {/* Emergency Pulse Hub */}
         <div className="mt-16 md:mt-40 p-6 md:p-24 bg-[#0a1628] rounded-2xl md:rounded-[5rem] text-white relative overflow-hidden group border-l-8 md:border-l-[16px] border-red-500 shadow-4xl">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12 md:gap-20">
               <div className="space-y-8 md:space-y-12 max-w-2xl text-center lg:text-left">
                  <div className="space-y-6">
                     ...
                     <h2 className="font-syne font-black text-3xl md:text-8xl leading-tight md:leading-[0.9] tracking-normal md:tracking-tighter uppercase italic break-words">{t('emergencyAssistanceTitle')}</h2>
                     <p className="text-white/40 font-dm text-base md:text-2xl italic leading-relaxed">{t('emergencyAssistanceSub')}</p>
                  </div>
