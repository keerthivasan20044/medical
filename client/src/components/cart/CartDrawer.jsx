        ...
        <>
          ...
          >
            ...
            <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <div className="space-y-1">
                   <div className="flex items-center gap-3">
                      ...
                      <h2 className="font-syne font-black text-2xl text-[#0a1628]">My Cart <span className="text-gray-300">/</span> {totalQty}</h2>
                   </div>
                   <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest pl-9">Order Summary</p>
                </div>
                <div className="flex items-center gap-4">
                   <button onClick={() => { if(window.confirm('Clear all items?')) dispatch(clearCart()) }} className="text-[10px] font-black text-red-500 uppercase tracking-widest hover:underline">Clear Items</button>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-8 space-y-8">
                {items.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-8 space-y-8">
                     ...
                     <div className="space-y-2">
                        ...
                        <p className="text-sm text-gray-400 font-dm">Looks like you haven't added any medicines yet.</p>
                     </div>
                     <Link to="/medicines" onClick={onClose} className="px-10 py-4 bg-[#0a1628] text-white rounded-2xl font-syne font-black text-[10px] uppercase tracking-widest hover:bg-brand-teal transition shadow-xl">
                        Browse Medicines
                     </Link>
                  </div>
               ) : (
                       ...
                       <div className="flex-1 space-y-3">
                          <div className="flex justify-between items-start">
                             <div>
                              ...
                              <button onClick={() => dispatch(removeFromCart(item._id || item.id))} className="text-gray-200 hover:text-red-500 transition-colors p-1">
                                 <Trash2 size={18}/>
                              </button>
                           </div>
                           ...
                           <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100">
                                 <button onClick={() => dispatch(decrementQty(item._id || item.id))} className="text-gray-400 hover:text-[#0a1628] transition p-1"><Minus size={14}/></button>
                                 <span className="font-syne font-black text-sm text-[#0a1628] w-4 text-center">{item.qty}</span>
                                 <button onClick={() => dispatch(incrementQty(item._id || item.id))} className="text-gray-400 hover:text-[#0a1628] transition p-1"><Plus size={14}/></button>
                              </div>
             ...
             {/* Footer Summary */}
             <div className="p-8 bg-white border-t border-gray-50 space-y-8 pb-32">
                <div className="space-y-4">
                   ...
                   <div className="flex justify-between items-center pt-2">
                      <span className="font-syne font-black text-xl text-[#0a1628] whitespace-nowrap overflow-hidden text-ellipsis mr-4">Total</span>
                      <span className="font-syne font-black text-3xl text-brand-teal shrink-0">₹{(totalAmount || 0).toLocaleString()}</span>
                   </div>
                </div>
                ...
                <div className="space-y-4">
                   ...
                   >
                      Proceed to Checkout <ArrowRight className="group-hover:translate-x-2 transition" size={18} />
                   </Link>
